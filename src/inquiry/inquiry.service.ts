...
//     };
//     await notificationService.createAndSendNotification(inquiry.userId, inquiryDto);
...
import InquiryRepository from './inquiry.repository'; // 변경된 클래스 import
import { InquiryList } from '../inquiry/dto/inquiry.dto';
import { InquiriesResponse, InquiryResponse } from 'src/product/dto/inquiry.dto';
import {
  BadRequestError,
...
export default class InquiryService {
  ...
  constructor(inquiryRepository: InquiryRepository, notificationService: NotificationService) {
  ...
  async getMyInquiries(
    ...
    if (role === 'SELLER') {
      const [list, totalCount] = await Promise.all([
        ...
        this.inquiryRepository.countInquiriesBySellerId(userId, { status: params.status }),
      ]);
    ...
    // buyer
    const [list, totalCount] = await Promise.all([
      this.inquiryRepository.findMyInquiryByUserId(userId, { skip, take, status: params.status }),
      this.inquiryRepository.countInquiryByUserId(userId, { status: params.status }),
    ]);
  ...
  async getDetailInquiry(userId: string, inquiryId: string): Promise<InquiriesResponse> {
    const inquiry = await this.inquiryRepository.findInquiryByInquiryId(inquiryId);
    if (!inquiry) {
      throw new NotFoundError();
    }
  ...
  async updateInquiry(
    ...
    const updateInquiry = await this.inquiryRepository.updateInquiry(inquiryId, data);
    return {
      id: updateInquiry.id,
      userId: updateInquiry.userId,
      productId: updateInquiry.productId,
  ...
  async postInquiryReply(
    ...
    try {
      ...
      await this.notificationService.createAndSendNotification(inquiry.userId, inquiryDto);
    } catch (error) {
